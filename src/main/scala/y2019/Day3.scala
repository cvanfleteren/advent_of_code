...
object Day3 extends App {
  ...
  def strictlyBetween(from: Int, to: Int)(toCheck: Int): Boolean = {
    val realFrom = Math.min(from, to)
    val realTo = Math.max(from, to)
    realFrom < toCheck && toCheck < realTo
  }

  case class Line(fromX: Int, fromY: Int, toX: Int, toY: Int, distance: Int) {

    def crosses(line: Line): Option[(Int, Int, Int)] = {
      // this horizontal
      if (strictlyBetween(fromX, toX)(line.fromX) && strictlyBetween(line.fromY, line.toY)(fromY)) {
        val myDistance = distance - Math.abs(toX - line.fromX)
        val lineDistance = line.distance - Math.abs(line.toY - fromY)
        Some(line.toX, fromY, myDistance + lineDistance)
      }else if (strictlyBetween(fromY, toY)(line.fromY) && strictlyBetween(line.fromX, line.toX)(fromX)) {
        // other line horizontal
        val myDistance = distance - Math.abs(toY - line.fromY)
        val lineDistance = line.distance - Math.abs(line.toX - fromX)
        Some(fromX, line.toY, myDistance + lineDistance)
      } else {
        None
      }
    }

    def step(dir: Char, amount: Int): Line = {
      dir match {
        case 'U' => Line(toX, toY, toX, toY + amount, distance = distance + amount)
        case 'D' => Line(toX, toY, toX, toY - amount, distance = distance + amount)
        case 'L' => Line(toX, toY, toX - amount, toY, distance = distance + amount)
        case 'R' => Line(toX, toY, toX + amount, toY, distance = distance + amount)
      }
    }
  }


  def mkLines(instructions: String): List[Line] = {

    val allLines = instructions.split(',').foldLeft(List(Line(0, 0, 0, 0, 0))) { case (lines, instruction) =>
      val dir = instruction.charAt(0)
      val amount = instruction.substring(1).toInt
      lines.head.step(dir, amount) :: lines
    }
    allLines.reverse.tail
  }


  def findAnswers(line1: String, line2: String): (Int, Int) = {
    val lines1 = mkLines(line1)
    val lines2 = mkLines(line2)

    val intersections = lines1.flatMap { l1 =>
      lines2.collect {
        case x if x.crosses(l1).isDefined => x.crosses(l1)
      }
    }.flatten

    val manhattanDistance = intersections.minBy(x => Math.abs(x._1) + Math.abs(x._2))
    val steps = intersections.minBy(x => Math.abs(x._3))
    (Math.abs(manhattanDistance._1) + Math.abs(manhattanDistance._2), steps._3)
  }

  //tests
  println(findAnswers("R8,U5,L5,D3", "U7,R6,D4,L4"))
  println(findAnswers("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83"))
  println(findAnswers("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"))

  //part1 && part2
  val answers = findAnswers(
