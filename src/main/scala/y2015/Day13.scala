...
object Day13 extends App {
  ...
  val in =
    s"""Alice would lose 57 happiness units by sitting next to Bob.
  ...
  def parseLine(line:String): (String, String, Int) = {
    ...
    parts match {
      case left :: _ :: direction :: amount :: tail => (left, tail.last, amount.toInt * sign(direction))
    }
  }


  def parseLines(in:String): Map[(String,String), Int] = {
    val defs = in.split('\n').map(parseLine).map(x => (x._1, x._2) -> x._3).toMap
    defs
  }

  def addSelfToDefs(defs:Map[(String,String), Int]): Map[(String,String), Int] = {
    val persons = defs.keySet.flatMap(x => Set(x._1, x._2)).toList
    persons.foldLeft(defs) { (defs, person) =>
      defs + (("self", person) -> 0) +  ((person, "self") -> 0)
    }
  }


  def solve(defs:Map[(String,String),Int]): Int = {
    val persons = defs.keySet.flatMap(x => Set(x._1, x._2)).toList

    //add self to defs

    persons.permutations.map{ arragnment =>
      //handle the "wrap around" (make it circular)
      (arragnment.last :: arragnment).sliding(2).map { case a::b::Nil =>
        val amount1 = defs((a,b))
        val amount2 = defs((b,a))
        amount1 + amount2
      }.sum
    }.max
  }
  ...
  def calc(in:String):Int = {
    val defs = parseLines(in)
    solve(defs)
  }

  def calc2(in:String):Int = {
    val defs = parseLines(in)
    solve(addSelfToDefs(defs))
  }
