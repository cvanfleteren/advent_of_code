...
object Day6 extends App {
  ...
  val in =
    s"""turn on 887,9 through 959,629
  ...
  def xy(in: String): (Int, Int) = {
    val parts = in.split(',')
    (parts.head.toInt, parts.last.toInt)
  }

  def parseInstruction(instruction: String): (Op, (Int, Int), (Int, Int)) = {
    val parts = instruction.split(' ').toList
    parts match {
      case "toggle" :: from :: _ :: to :: Nil => (Toggle, xy(from), xy(to))
      case "turn" :: "on" :: from :: _ :: to :: Nil => (On, xy(from), xy(to))
      case "turn" :: "off" :: from :: _ :: to :: Nil => (Off, xy(from), xy(to))
    }
  }

  def xyToIndexGeneric(columns: Int)(xy: (Int, Int)): Int = {
    xy._1 * columns + xy._2
  }


  def newValuePart2(op: Op, curValue:Int): Int = {
    op match {
      case Toggle => curValue + 2
      case On => curValue + 1
      case Off => Math.max(curValue - 1, 0)
    }
  }

  def newValuePart1(op: Op, curValue:Int): Int = {
    op match {
      case Toggle => if(curValue == 1) 0 else 1
      case On => 1
      case Off => 0
    }
  }

  def count(in: String, newValue: (Op, Int) => Int, columns:Int = 1000): Int = {

    val xyToIndex = xyToIndexGeneric(columns) _

    def process(instructions: List[(Op, (Int, Int), (Int, Int))]): Map[Int,Int] = {
      // use a map as a Sparse Matrix, less memory vs full List/Arrays
      val state = scala.collection.mutable.Map[Int,Int]().withDefaultValue(0)
      instructions.foreach { instruction =>
        val indexes = for {
          x <- Range.inclusive(instruction._2._1, instruction._3._1).toList
          y <- Range.inclusive(instruction._2._2, instruction._3._2).toList
        } yield (xyToIndex((x, y)))

        indexes.foreach { index =>
          state(index) = newValue(instruction._1, state(index))
        }
      }
      state.toMap
    }
    ...
    process(instructions).values.sum
  }

  println(count(in, newValuePart1))
  println(count(in, newValuePart2))
}
