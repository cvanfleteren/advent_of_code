...
object Day8 extends App {
  ...
  val in =
    s""""/xa8br/x8bjr/""
       ...
       |"c//naawulxlqplnacvytspry/xf5ytxxqq"
       |"razwqmsqgbaaxcd//f"
       |"radggyrjrg/"zx"
       ...
       |"rubahvmp/""
       |"qmkukrnrmqumh"
       |"wdpxyvyidhwjf//nabbijwhr/xc5bksvy/"p"
       |"u/"prlpg/""
       |"nsvcquyxbwilsxxemf/xd9leq"
       |"y/xcetxuafl"
       |"it"
       |"kwdlysf//xjpelae"
       |"viwh/x58wpjjlnvryuti/x2chngrx//nhtkui"
       ...
       |"vtfi"
       |"//h"
       |"qekfrc/xf6wduodbwrguqcng//n"
       |"/"lljlfdrxftwidn//pkv/xd9ij"
       |"mrvgqynpehkliuijlpp"
       |"gikjph"
       |"yoxcdrdt/"wbaurnyhoyxoihu"
       |"onmomwuxuammbzxe"
       |"rnrr//twviz/x61gqaljr/x0dmtw"
       |"r/"vupaoi"
       |"l"
       |"sei"
       |"jwxtdtbkd//kxd"
  ...
  def decode(line:String) : String = {
    val l1 = line.replaceAll("""\\x[0-9,a-f]{2}""","*")
    val l2 = l1.replace(slash+slash,"/")
    val l3 = l2.replace(slash,"")

    l3.substring(1, l3.length -1)
  }

  def encode(line:String): String = {
    val l1 = line.replace(slash,slash+slash)
    val l2 = l1.replace(quote,slash+quote)

    quote + l2 + quote
  }


  val part1CodeSize = in.split('\n').map(_.length).sum
  val part1ValueSize = in.split('\n').map(decode(_).length).sum
  println(part1CodeSize - part1ValueSize)

  val part2Codesize = in.split('\n').map(encode(_).length).sum
  println(part2Codesize - part1CodeSize)
}
