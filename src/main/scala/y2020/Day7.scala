...
object Day7 extends App {
  ...
  val in =
    s"""plaid magenta bags contain 2 clear lavender bags, 3 clear teal bags, 4 vibrant gold bags.
  ...
  def parseRules(rulesDef: String): (Map[String, List[String]], Map[String, List[ColorDef]]) = {
    val allDefs = rulesDef.split('\n').flatMap(parseLine).toList


    // map from color to colors it can contain (and the counts)
    val mapped = allDefs.groupBy(_._1).view.mapValues(_.map(_._2)).toMap

    // map from color to colors it can be contained by
    val inverseMapped = allDefs.groupBy(_._2.color).view.mapValues { defs =>
      defs.map(_._1)
    }.toMap
    (inverseMapped, mapped)
  }

  case class ColorDef(color:String, amount:Int)

  def parseLine(line: String): List[(String, ColorDef)] = {
    if (line.contains("contain no ")) {
      Nil
    } else {
      val words = line.split(' ')
      val firstColor = words(0) + " " + words(1)

      val canContain = line.drop(line.lastIndexOf("contain") + 8).split(", ").map { colorPart =>
        val parts = colorPart.split(' ')
        val color = parts(1) + " " + parts(2)
        ColorDef(color, parts(0).toInt)
      }

      canContain.toList.map { colorDef =>
        firstColor -> colorDef
      }
    }
  }

  def canBeContainedByDirectAndIndirect(color: String, rules: Map[String, List[String]]): Int = {

    def containCountPath(color: String, found: Set[String]): Set[String] = {
      val canBeContainedBy = rules.get(color)
      canBeContainedBy match {
        case None => found
        case Some(colors) =>
          colors.flatMap { color =>
            containCountPath(color, found + color)
          }.toSet
      }
    }

    val all =  containCountPath(color, Set.empty)
    all.size
  }

  def totalBagCount(color:String, rules: Map[String, List[ColorDef]]): Int = {

    val root = rules.get(color)

    root match {
      case None => 0
      case Some(root) =>
        root.map { colorDef =>
          colorDef.amount + (colorDef.amount * totalBagCount(colorDef.color, rules))
        }.sum
    }
  }

  val (inverseRules,rules) = parseRules(in)

  val part1 = canBeContainedByDirectAndIndirect("shiny gold", inverseRules)
  println(part1)

  val part2 = totalBagCount("shiny gold", rules)
  println(part2)
