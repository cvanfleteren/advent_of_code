...
object Day4 extends App {
  ...
  def isInRange(min:Int, max:Int)(value:String): Boolean = {
    !value.exists(_.isDigit == false) && value.toInt >= min && value.toInt <= max
  }

  def isValidHeight(height:String): Boolean = {
    if(height.endsWith("cm")) {
      isInRange(150, 193)(height.filter(_.isDigit))
    } else if(height.endsWith("in"))  {
      isInRange(59, 76)(height.filter(_.isDigit))
    } else {
      false
    }
  }
  ...
  val  isValidByr = isInRange(1920, 2002) _
  val  isValidIyr = isInRange(2010, 2020) _
  val  isValidEyr = isInRange(2020, 2030) _

  def isValidPart1(passport: Map[String,String]): Boolean = {
    passport.keySet.intersect(requiredFields).size == requiredFields.size
  }


  def isValidPart2(passport: Map[String,String]): Boolean = {
  ...
  def parsePassports(in:String): Iterable[Map[String,String]] = {
    in.split("\n{2}").map { lines =>
      val passport = lines.split("\\s").toList.map { keyValue =>
        val pairs = keyValue.split(":")
        (pairs(0) -> pairs(1))
      }.toMap
  ...
  val part1 = parsePassports(in).count(isValidPart1)
  val part2 = parsePassports(in).count(isValidPart2)
